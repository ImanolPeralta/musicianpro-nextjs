...
export default function CartPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F5EFE6] p-6 sm:p-10">
      <h1 className="text-3xl font-bold text-[#1C1C1C] mb-8 text-center">Tu carrito</h1>
              ...
              >
                <div className="flex items-center gap-4 w-full">
                  ...
                  <div className="flex flex-col">
                    <h2 className="text-lg font-semibold text-[#1C1C1C]">{item.titulo}</h2>
                    <p className="text-sm text-[#1C1C1C]">
                      ${item.precio.toLocaleString("es-AR")} x {item.cantidad} unidad{item.cantidad > 1 ? "es" : ""}
                    </p>
                  </div>
                </div>
  ...
  <div className="flex items-center gap-2">
    <button
      onClick={() =>
        item.cantidad > 1 && updateQuantity(item.id, item.cantidad - 1)
      }
    ...
    <div className="flex flex-col items-center">
      ...
      {item.cantidad >= item.stock && (
        <span className="text-[10px] text-red-600">Máximo alcanzado</span>
      )}
    </div>

    <button
      onClick={() =>
        item.cantidad < item.stock && updateQuantity(item.id, item.cantidad + 1)
      }
      className="bg-[#8B2C2C] text-white p-1 rounded-full hover:bg-[#641B1B] transition cursor-pointer"
      aria-label="Aumentar"
    >
  ...
  >
    <HiOutlineTrash size={20} />
          ...
          {/* Resumen de compra */}
          <div className="bg-[#E8DCCB] p-6 rounded-2xl shadow-md flex flex-col justify-between h-fit"
            style={{ boxShadow: `0 4px 12px ${"#B0AFAF"}` }}
          >
            <h2 className="text-xl font-semibold mb-4 text-[#1C1C1C]">Resumen</h2>
